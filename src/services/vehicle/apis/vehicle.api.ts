...
import type { Vehicle, VehicleBrand, VehicleModel } from "@/types/models/vehicle";
...
export const getVehicleBrands = async () => {
    return await httpPrivate.get<BaseResponse<{ data: VehicleBrand[] }>>("/vehicles/brands");
}

export const getVehicleModelsByBrand = async (brand: number | string) => {
    return await httpPrivate.get<BaseResponse<{ data: VehicleModel[] }>>(`/vehicles/brands/${brand}/models`);
}
